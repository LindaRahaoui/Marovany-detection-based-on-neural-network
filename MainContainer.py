...
class MainContainer(QWidget):
    ...
    def call_ChooseFileDialog_save_annotations(self):
    ...
    def save_notes_to_excel(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(parent=None, caption="Save MIDI notes to Excel", directory="", filter="Excel Files (*.xlsx)")
        if file_path:
    ...
    def save_annotations_to_csv(self): 
        ...
        onsets = [note.start for note in midi_notes]
        offsets = [note.end for note in midi_notes]
        midi_note = [note.pitch for note in midi_notes]
        # Pour chaque midi note convertir en pitch frequency
        pitch = [pretty_midi.note_number_to_hz(note.pitch) for note in midi_notes]

        # Vérifier que les onsets et offsets ont la même longueur
        assert len(onsets) == len(offsets), "Les listes d'onsets et d'offsets doivent avoir la même longueur."
        
        # Définir les données du CSV
        data = {
            'onset': onsets,
            'offset': offsets,
            'pitch': pitch,
        ...
        df = pd.DataFrame(data)
        # Supprimer les lignes où midi_note=0
        df = df[df['midi_note'] != 0]
        
        # Trier le DataFrame par la colonne 'onset'
        df_sorted = df.sort_values(by='onset')
        
        # Demander à l'utilisateur de sélectionner un fichier WAV pour récupérer le chemin de base
        file_path, _ = QFileDialog.getOpenFileName(parent=None, caption="Select the base WAV file", directory="", filter="Audio Files (*.wav)")
        
        if file_path:
            # Créer le répertoire "prédictions" s'il n'existe pas déjà
            annotations_dir = os.path.join(os.path.dirname(file_path), 'annotations')
            os.makedirs(annotations_dir, exist_ok=True)
            
            # Obtenir le nom de base du fichier sans extension et ajouter "_prediction.csv"
            base_name = self.analysis_widget.fname.split('.')[0]
            annotation_file_name = f"{base_name}_annotation.csv"
            annotation_file_path = os.path.join(annotations_dir, annotation_file_name)
            
            try:
                # Sauvegarder les annotations dans le fichier CSV
                df_sorted.to_csv(annotation_file_path, index=False)
                self.save = False
                self.Clear_Midi_Notes(self)
                QMessageBox.information(self, "Save Annotations", "Annotations saved successfully.")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"An error occurred while saving the annotations: {e}")
